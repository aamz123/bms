...
export class Cell {
  ...
  getCellChargeColor(): string {
    ...
    if (this.stateOfCharge >= 20 && this.stateOfCharge < 40)
      return "bg-[#FFA94C]"; // Orange
    if (this.stateOfCharge >= 40 && this.stateOfCharge < 60)
      return "bg-[#FFD700]"; // Gold
    if (this.stateOfCharge >= 60 && this.stateOfCharge < 80)
      return "bg-[#A3E635]"; // Lime Green
  ...
  calculateDischargingBAC(): number {
    return (
      (((1 / (this.temperature + 1)) * 1) / (this.numberOfChargeCycles + 1)) *
      this.stateOfCharge
    );
  }
